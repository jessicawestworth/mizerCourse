...
In the previous tutorial we plotted the community size spectra and how it related to Sheldon’s observation. At the end of the tutorial we then plotted single-species size spectra from our dataset and observed that they were a bit of a mess. In this tutorial we will use the mizer model to gain an understanding of what determines the shape of a species size spectrum by plotting a species size spectrum within a non-dynamic background community following Sheldon's observed size spectra.
...
Additionally, from this point forward in the course, biomass density in log weight will often be referred to as simply biomass density for brevity. To determine which meaning is intended in a given context, refer to the y-axis of the accompanying figure: if units [g] are provided, the term refers to biomass density in log weight; if no units are shown, we are referring to biomass density in the non-log sense.  If this does not sound familiar, please refer to the first tutorial where we denote the units of the different densities.
...
5\) As fish mature, they start investing some of their energy into reproduction and hence their growth slows. This leads to a bump in the size spectrum. We saw how changes in the parameters describing the maturity change the shape of that bump.
