...
4\)  The number density looks very different from the biomass density. There will be a lot of very small individuals, so the number density at small sizes will be large, but each individual weighs little, so their total biomass will not be large. 

5\)  A community size spectra refers to the distribution of abundance or biomass across different body sizes within an entire ecological community. Community size spectra are approximately power laws. When displayed on log-log axes, they look like straight lines. The slope of the number density is approximately -2, the slope of the biomass density is approximately -1.
...
8\)  Unlike the community spectrum, the size spectra of an individual species are not approximate power laws and thus do not appear as straight lines on log-log axes. The approximate power law only emerges when we add all the species spectra together.
...
This finding is fundamental in size spectrum modelling, as it provides a way to represent the background community in which a species lives and feeds upon. Sheldon’s observation, that the exponent lambda of the power-law relationship between body size and biomass density in log weight is approximately 2 across all marine ecosystems, allows us to approximate the size structure of this background community without requiring high-resolution species-specific data. This generalization makes it easy to embed one or more focal species into an ecologically realistic background community.  In the next tutorial, this concept will become more concrete as we explore a model of a single species living within an unchanging background community set to a Sheldon power-law abundance.
